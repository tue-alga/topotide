...
InputGraph::InputGraph(const HeightMap& heightMap,
	...
	struct Edge {
		...
		static int directionBetween(HeightMap::Coordinate p1,
			...
			if (p2.m_x == p1.m_x + 1) {
				if (p2.m_y == p1.m_y) {
					return 0;
				} else if (p2.m_y == p1.m_y + 1) {
					return 5;
				}
			} else if (p2.m_x == p1.m_x) {
				if (p2.m_y == p1.m_y - 1) {
					return 1;
				} else if (p2.m_y == p1.m_y + 1) {
					return 4;
				}
			} else if (p2.m_x == p1.m_x - 1) {
				if (p2.m_y == p1.m_y - 1) {
					return 2;
				} else if (p2.m_y == p1.m_y) {
					return 3;
				}
			}
			return -1;
	...
	std::vector<std::vector<std::array<bool, 6>>> topEdges(
	            heightMap.width(),
	            std::vector<std::array<bool, 6>>(heightMap.height(),
	                                             std::array<bool, 6>{}));
	std::vector<std::vector<std::array<bool, 6>>> bottomEdges(
	            heightMap.width(),
	            std::vector<std::array<bool, 6>>(heightMap.height(),
	                                             std::array<bool, 6>{}));
	std::vector<std::vector<std::array<bool, 6>>> sourceEdges(
	            heightMap.width(),
	            std::vector<std::array<bool, 6>>(heightMap.height(),
	                                             std::array<bool, 6>{}));
	std::vector<std::vector<std::array<bool, 6>>> sinkEdges(
	            heightMap.width(),
	            std::vector<std::array<bool, 6>>(heightMap.height(),
	                                             std::array<bool, 6>{}));
	...
	while (!edges.empty()) {
		...
		for (int i = 0; i < 6; i++) {
			int direction = (i + startDirection) % 6;
			...
			std::cout << "    edges: " << onTopOrBottom << " " << onSource
			          << " " << onSink << std::endl;*/
			...
			if (inside) {
				if (!marked[destination.m_x][destination.m_y]) {
					...
					edges.push(Edge(destination, (direction + 2) % 6));
				}
			}
		}
...
VertexType InputGraph::vertexType(int i) const {

	if ((*this)[i].adj.size() == 0) {
		return VertexType::disconnected;
	}

	Wedges wedges = getWedges(i);

	if (wedges.ascending.size() == 0 && wedges.descending.size() == 1) {
		return VertexType::maximum;
	}

	if (wedges.ascending.size() == 1 && wedges.descending.size() == 0) {
		return VertexType::minimum;
	}

	assert(wedges.ascending.size() == wedges.descending.size());

	if (wedges.ascending.size() == 1) {
		return VertexType::regular;
	}

	return VertexType::saddle;
}

InputGraph::Wedges InputGraph::getWedges(int i) const {

	Wedges w;

	if (m_verts[i].adj.size() == 0) {
		return w;
	}

	const Vertex v = (*this)[i];
	const std::vector<Adjacency>& adj = (*this)[i].adj;

	// find start of an ascending wedge
	int start = 0;
	for (int i = 0; i < adj.size(); i++) {
		if (!isAscending(adj[(i + adj.size() - 1) % adj.size()]) &&
				isAscending(adj[i])) {
			start = i;
			break;
		}
	...
	do {
		// ascending
		if (isAscending(adj[i])) {
			int iSteepest = i;
			do {
				if (compareSteepness(adj[iSteepest], adj[i])) {
					iSteepest = i;
				}
				i = (i + 1) % adj.size();
			} while (isAscending(adj[i]) && i != start);
			w.ascending.push_back(iSteepest);
		}
		...
		if (!isAscending(adj[i])) {
			int iSteepest = i;
			do {
				if (compareSteepness(adj[i], adj[iSteepest])) {
					iSteepest = i;
				}
				i = (i + 1) % adj.size();
			} while (!isAscending(adj[i]) && i != start);
			w.descending.push_back(iSteepest);
		}
	} while (i != start);
