 ...
 * are each represented by a Path. In clockwise order, these are:
 *
 * * the source,
 * * the top side,
 * * the sink, and
...
class Boundary {
	...
	public:
		...
		/**
		 * Constructs the default boundary for the given heightmap, that is, one
		 * that has the source on the left side and the sink on the right side,
		 * and spans the entire river.
		 *
		 * \param map The heightmap to construct the default boundary for.
		 */
		Boundary(HeightMap map);
